...
```{r}
blocks <- st_read("~/Downloads/Census_Blocks.shp")
...
```{r}
baltimore_city_blocks %>%
  filter(Adj_H18__P > Adj_NH18__ & Adj_H18__P > Adj_NH18_1) 
```
...
Well, Census blocks where adult Hispanic population outnumbers both white and Black adult population are clustered in the city's eastern and southwestern areas. There are fewer such blocks in the northern parts of the city.

This is a pretty blunt visualization. Not all Census blocks are equally sized. Some have more people than the others, for example.

We can get a sense of where the largest Hispanic populations are, by changing the color of the shapes according to the number of people. We do this by setting the aesthetic -- or aes -- to use the Adj_H18__P column inside of the geom_sf function.  To make the differences easier to see, we're going to change the fill of the city white, too, and use a special color palette, viridis magma. We're also going to make the shapes slightly bigger.
...
ggplot() +
  ...
  geom_sf(data=baltimore_hispanic_blocks, aes(color=Adj_H18__P), size=2) +
