...
Let's dive back into zip codes and this time bring along August 2024 data on Maryland registered voters' party affiliation by zip code. Let's load it.
...
What if we wanted to find the zip codes that were closely divided between Democrats, Republicans and unaffiliated (independent) voters? For that, we can use a measure called the Gini Coefficient. Usually it's used to gauge inequality, but it also can be used to show how alike, or equal, things are. We'll need to install a package to use it:

### Task: Install ineq library
...
Then we'll use the following code to calculate the zip codes with the most equal and unequal distribution of percentages for the three categories (PCT_DEM, PCT_REP and PCT_UNA). The `rowwise()` function allows us to calculate the coefficient on each row, and `ungroup()` returns the dataframe to its original state after we do the calculation. Then we sort by the smallest coefficient to find the zip codes with the most similar distributions of voters:

**Task** Run the following code

```{r}
md_voters_zip_gini <- zip_codes_with_voters |> 
  rowwise() |> 
  mutate(gini_coefficient = ineq(c(PCT_DEM, PCT_REP, PCT_UNA), type="Gini")) |> 
  ungroup() |> 
  arrange(gini_coefficient) |> 
  select(ZIPCODE1, DEM, REP, UNA, TOTAL, PCT_DEM, PCT_REP, PCT_UNA, gini_coefficient, geometry)

```

Finally, let's put that on a map, using `scale_fill_viridis_b()` to make the distinctions between the values clearer. Remember, a darker color means more similarity.

**Task** Run the following code and describe the results below. Where are the more similar zip codes?

**Answer**

```{r}
ggplot() +
  geom_sf(data=md_voters_zip_gini, aes(fill=gini_coefficient)) +
...
**Task** Run the following code. Describe the output in the space below. Be sure to input your census api key if you haven't already. **Answer**
...
Let's take a look at the result of this table. The variances in the rates aren't huge, but there are some clear differences: Frederick is the clear leader, followed by Howard and Anne Arundel. Prince George's has the second-smallest rate of unaffiliated voters in the state.
