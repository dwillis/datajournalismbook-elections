...
standardized_maryland_cities <- maryland_cities |>
  #with a new column
  mutate(
    # named upper_city, which has the values from the "city" column but in upprcase instead
...
maryland_winred_categories <- maryland_winred_contributions |>
  ...
  mutate(
    ...
    amount_category = case_when(
      # when the value in the amount column is less that 100, make the value in our new amount_category column "Less than $100"
...
prince_georges <- primary_24 |>
  # filter the data to only the rows where county_name is LITERALLY/exactly Prince George's County
...
or_prince_georges <- primary_24 |>
  # give me the rows where the county name is Prince Georgie's County OR where the office name is U.S. Senator
  filter(county_name == "Prince George's County" | office_name == "U.S. Senator")
