...
Let's load some Maryland individual campaign contribution data.

```{r include=FALSE}

maryland_house_contribs <- read_csv("data/maryland_house_contribs.csv")

```

Now let's try and group and count the number of contributions by city. To make it a bit more manageable, let's use another string function from `stringr` and filter for cities that start with the uppercase "B" or lowercase "b" using the function `str_detect()` with a regular expression.  

The filter function in the codeblock below says: look in the city column, and pluck out any value that starts with (the "^" symbol means "starts with") a lowercase "b" OR (the vertical "|", called a pipe, means OR) an uppercase "B".  

```{r}
maryland_house_contribs %>%
  group_by(contributor_city) %>%
  summarise(
    count=n()
  ) %>%
  filter(str_detect(contributor_city, "^b|^B")) %>%
...
```{r}
cleaned_maryland_house_contribs <- maryland_house_contribs %>%
  mutate(city_clean=key_collision_merge(contributor_city)) %>%
  select(transaction_id, contributor_city, city_clean, everything())

cleaned_maryland_house_contribs

```
To examine changes `refinr` made, let's examine the changes it made to cities that start with the letter "B".

```{r}
cleaned_maryland_house_contribs %>%
  group_by(contributor_city, city_clean) %>%
  summarise(
    count=n()
  ) %>%
  filter(str_detect(city_clean, "^b|^B")) %>%
  arrange(city_clean)
```
Hmm, no apparent changes. Key collision will do well with different cases, but all of our records are upper case.
...
```{r}
cleaned_maryland_house_contribs <- maryland_house_contribs %>%
  mutate(city_clean=n_gram_merge(contributor_city)) %>%
  select(transaction_id, contributor_city, city_clean, everything())
