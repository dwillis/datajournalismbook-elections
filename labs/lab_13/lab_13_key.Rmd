...
results <- "https://www.congress.gov/sponsors-cosponsors/117th-congress/representatives/ALL" %>% 
  read_html() %>% 
  html_table()
...
members <- results[[1]] %>% 
  clean_names() %>% 
  slice(c(-1:-2)) %>% 
  rename(member = 1, bills_sponsored = 2, amendments_sponsored = 3, bills_cosponsored = 4, amendments_cosponsored = 5, withdrawn=6) %>% 
  mutate(across(c(2:6), as.numeric))

members %>% 
  summarize(avg_bills_sponsored = mean(bills_sponsored))
